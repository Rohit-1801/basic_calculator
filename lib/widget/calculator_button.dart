...
import 'package:fluttercalculator/model/calculator_value.dart';
...
class CalculatorButton extends StatelessWidget {
  ...
  void pressButton(String title, BuildContext context) {
    ...
    bool shouldCalculate = Provider.of<CalculatorValue>(context).shouldCalculate;

    if (title == "+" || title == "-" || title == "*" || title == "/") {
      if (shouldCalculate) {
        Provider.of<CalculatorValue>(context).calculateValue();
      } else {
        Provider.of<CalculatorValue>(context).result = double.parse(numberString) ?? 0;
